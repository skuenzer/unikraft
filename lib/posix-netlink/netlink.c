...
#include <uk/netlink_protocol.h>
#include <uk/socket_driver.h>
#include <uk/print.h>

#define sock2nlctx(s) \
	((struct nl_ctx *) posix_sock_get_data(s))
...
{
	const struct posix_netlink_protocol *drv = posix_netlink_protocol_list_start;
		       ...
		       int family __maybe_unused, int type, int protocol)
{
	...
	uk_pr_debug("Created netlink socket: %p, protocol: %d, drv: %s\n", nl_ctx,
		    nl_ctx->drv->protocol, nl_ctx->drv->libname);
	return nl_ctx;
			  ...
			  int flags __unused)
{
	...
	//UK_ASSERT(nl_msg->nlmsg_len >= sizeof(*nl_msg));

	uk_pr_debug("great - we continue\n");
			   ...
			   struct sockaddr *from, socklen_t *fromlen)
{
	...
	if (from && (!fromlen || (*fromlen < sizeof(*nl_addr))))
		return -EINVAL;
	...
	/* Write from address only if requested */
	if (from) {
...
static const struct posix_socket_ops netlink_vops = {
	...
	.poll		= nl_poll,
#if 0
	.accept4	= netlink_accept4,
	.shutdown	= netlink_shutdown,
	.getpeername	= netlink_getpeername,
	.getsockopt	= netlink_getsockopt,
	.setsockopt	= netlink_setsockopt,
	.connect	= netlink_connect,
	.listen		= netlink_listen,
